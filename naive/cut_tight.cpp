...
{
    _GG= new GG(1,1,Ec,T,omega_c,N1,1000);
...
double Iterator::Gamma(int i,int j, int k){
    ...
    double gg;
    if(k%2==0) {freqj=freq0;gg=_GG->result0(j);}
    else {freqj=freq1;gg=_GG->result1(j);}
    if(k/2==0) freqi=freq0;
    else freqi=freq1;
    //if(DEBUG) std::cout<<"end gamma"<<std::endl;    
    return -omega*omega//(freqi[i]-freqj[j])*(freqi[i]-freqj[j])
...
int Iterator::update1(int M){
    ...
    //if(DEBUG) std::cout<<"loop 1 end"<<std::endl;    
    ...
    if(DEBUG) std::cout<<_GG->result1(0)<<std::endl;    			
    //update result to delta1
    //if(DEBUG) std::cout<<"loop 2 end"<<std::endl;    
    for(int i=0;i<N1;i++){
	double d1=0;
	//if(DEBUG) std::cout<<_GG->result1(0)<<std::endl;    			
	for(int j=0;j<N1;j++){
...
int main(){
    double t=1.0/1023.0;
    for(int l=100;l<40000;l*=2){
	t=1.0/(l);
	std::cout<<t<<"\t"<<test(t,1.0)<<std::endl;
    }
    return 0;
}
