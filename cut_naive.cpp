...
public:
    Iterator(double,double,double,double,double);
...
Iterator::Iterator(double G, double O, double E, double t,double O_c)
    :g(G),omega(O),Ec(E),T(t),omega_c(O_c)
{
...
int Iterator::update1(int M){
	...
	for(int j=0;j<N1;j++){
	    for(int k=0;k<N1;k++)
		sum_delta[j]+= -Gamma(j,k,3)*sum_delta[k]/i;
